...
def intersect_polygons_with_events(P, Q):
    ...
    # Initialize sweep line status (active edges)
    active_P_upper = None
    active_P_lower = None
    active_Q_upper = None
    active_Q_lower = None

    # Initialize event queue (list of potential next events)
    event_queue = []
    intersection_points = []
    event_log = []
    ...
    # Find the leftmost x-coordinate to start the sweep
    all_points = [p for p, _ in P_upper + P_lower + Q_upper + Q_lower]
    min_x = min(p.x for p in all_points)

    # Add initial events (leftmost points of each chain)
    if P_upper_idx < len(P_upper) - 1:
        seg = Segment(P_upper[P_upper_idx][0], P_upper[P_upper_idx + 1][0])
        active_P_upper = (seg, P_upper_idx)
        event_queue.append((seg.p2.x, 'PU', P_upper_idx + 1))
    if P_lower_idx < len(P_lower) - 1:
        seg = Segment(P_lower[P_lower_idx][0], P_lower[P_lower_idx + 1][0])
        active_P_lower = (seg, P_lower_idx)
        event_queue.append((seg.p2.x, 'PL', P_lower_idx + 1))
    if Q_upper_idx < len(Q_upper) - 1:
        seg = Segment(Q_upper[Q_upper_idx][0], Q_upper[Q_upper_idx + 1][0])
        active_Q_upper = (seg, Q_upper_idx)
        event_queue.append((seg.p2.x, 'QU', Q_upper_idx + 1))
    if Q_lower_idx < len(Q_lower) - 1:
        seg = Segment(Q_lower[Q_lower_idx][0], Q_lower[Q_lower_idx + 1][0])
        active_Q_lower = (seg, Q_lower_idx)
        event_queue.append((seg.p2.x, 'QL', Q_lower_idx + 1))
    ...
    while event_queue:
        ...
        event_log.append((x, [active_P_upper[0] if active_P_upper else None, active_P_lower[0] if active_P_lower else None],
                         [active_Q_upper[0] if active_Q_upper else None, active_Q_lower[0] if active_Q_lower else None],
        ...
        active_segments_Q = []
        if active_P_upper:
            active_segments_P.append(active_P_upper[0])
        if active_P_lower:
            active_segments_P.append(active_P_lower[0])
        if active_Q_upper:
            active_segments_Q.append(active_Q_upper[0])
        if active_Q_lower:
            active_segments_Q.append(active_Q_lower[0])
        ...
        for seg_P in active_segments_P:
            for seg_Q in active_segments_Q:
                ...
                if inter and inter not in intersection_points and inter not in new_intersections:
                    if inter.x >= x:  # Only add intersection if it's to the right of the current sweep line
                        new_intersections.append(inter)

        # Add new intersection points to the event queue and intersection_points list
...
def animate_sweep_line(P, Q, event_log, title):
    ...
    # Prepare the event queue for visualization (will be updated dynamically)
    event_x_coords = []
    event_labels = []

    # Initialize event queue visualization on ax2
    ax2.set_title('Event Queue (Next Events)')
    ax2.set_xlim(0, 1)
    ax2.set_yticks([])
    ax2.set_xticks([])
    ...
    def update(frame):
        nonlocal intersection_fill, event_x_coords, event_labels
        ...
        event_x_coords = []
        event_labels = []
        # Add right endpoints of active segments
        if frame < len(event_log) - 1:
            next_events = event_log[frame + 1][0]  # Look at the next frame's events
            # We need to reconstruct the event queue based on the events that were added
            # Since event_log doesn't store the event queue directly, we infer it from the active segments
            active_segs = []
            if segs_P[0]:  # P upper
                active_segs.append((segs_P[0].p2.x, 'PU'))
            if segs_P[1]:  # P lower
                active_segs.append((segs_P[1].p2.x, 'PL'))
            if segs_Q[0]:  # Q upper
                active_segs.append((segs_Q[0].p2.x, 'QU'))
            if segs_Q[1]:  # Q lower
                active_segs.append((segs_Q[1].p2.x, 'QL'))
            ...
            if frame > 0:
                ...
                for i, inter in enumerate(new_inters):
                    if inter.x >= x:
                        active_segs.append((inter.x, f'I{i}'))
            active_segs.sort()
            for x_coord, label in active_segs:
                if x_coord >= x:
                    found = False
                    for i, existing_x in enumerate(event_x_coords):
                        if abs(existing_x - x_coord) < 1e-9:
                            event_labels[i] += f", {label} ({x_coord:.2f})"
                            found = True
                            break
                    if not found:
                        event_x_coords.append(x_coord)
                        event_labels.append(f"{label} ({x_coord:.2f})")
    ...
    ani = animation.FuncAnimation(fig, update, frames=len(event_log), init_func=init, blit=True, interval=500)
    return fig, ani
